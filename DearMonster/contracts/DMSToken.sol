...
contract DMSToken is Context, IDMSToken {
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[_sender][_msgSender()].sub(
                _amount,
                "DMST: transfer amount exceeds allowance"
            )
        );
        return true;
    }
    ...
    function _transfer(
        ...
        require(_sender != address(0), "DMST: transfer from the zero address");
        require(_recipient != address(0), "DMST: transfer to the zero address");

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "DMST: transfer _amount exceeds balance"
        );
    ...
    function _burn(address _account, uint256 _amount) internal {
        require(_account != address(0), "DMST: burn from the zero address");

        _balances[_account] = _balances[_account].sub(
            _amount,
            "DMST: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "DMST: approve from the zero address");
        require(_spender != address(0), "DMST: approve to the zero address");
